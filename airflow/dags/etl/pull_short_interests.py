            ...
            if last_dates != None:
                if symbol in last_dates:
                    ...
                    if a_before_b(date, PULL_DATE):
                        ...
                        if len(data)==0:
                            logger.warn("{}: last date in db ({}) is before pull date ({}) and data exist. Keep the data for storing".format(symbol, date, PULL_DATE))
                        else:
                            logger.warn("{}: last date in db ({}) is before pull date ({}) but no data is available in Quandl".format(symbol, date, PULL_DATE))
                    else:
